...
- Changes APIs.
- Integrates complex external or AI agent libraries (e.g., Koog, Live API, external SDKs).
...
- Relevant specialised domain skills (e.g. `koog-agent-framework`, `android`, etc.) when working with specialized subsystems.
...
Data model
    ↓
Repository / Service / Agent Layer
    ↓
...
- UI regression.
- API compatibility.
- Non-determinism, timeouts, or rate limiting in external/agent integrations.
